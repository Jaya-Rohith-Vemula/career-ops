...
- Quantified metrics only belong in the two most recent EXPERIENCE entries. Every metric used must trace back to a real number/percentage from the base resume; distribute them across those two entries only, attaching each to whichever bullet there is most relevant to the target job's core requirements, even if that means moving a metric off the bullet it originally sat on and rewriting that bullet's technology around the new context. Don't leave the bullets that matter most for this job with no metric while a less relevant bullet in one of those two entries keeps one.
...
function main() {
  ...
  if (!companyId || !jobId) {
    console.error('Usage: node resume/FT/tailorResume.js --companyId=<id> --jobId=<id>');
    process.exit(1);
  }
